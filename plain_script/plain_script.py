...
# Helper function for RoPE (Rotary Positional Embeddings)
def __apply_rope(x, seq_len, head_dim):
    # x shape: (B, H, S, D)
    input_dtype = x.dtype
...
def mqa_rope(i, wq, wk, wv, wo, num_heads, num_kv_heads):
...
def transformer_block(x, params, num_heads, num_kv_heads):
...
def main():
    ...
    # step 5: lm head
    my_logits = lm_head(x, model.get_input_embeddings().weight.to(dtype))
    
    # step 6: Get ground truth logits from the original model
    with torch.no_grad():
        ground_truth_outputs = model(**model_inputs)
